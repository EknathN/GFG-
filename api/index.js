...
// Vercel handles environment variables automatically, no need for dotenv.config()
const SUPABASE_URL = process.env.VITE_SUPABASE_URL || process.env.SUPABASE_URL;
// Use SERVICE_ROLE key for server-side if available, fallback to anon
...
ENTITIES.forEach(entity => {
  ...
  app.get(`/api/${entity}`, async (req, res) => {
    if (!supabase) return res.status(503).json({ error: 'Database not connected (Check Supabase env vars)' });
    try {
